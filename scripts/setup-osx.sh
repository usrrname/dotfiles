...
source "$(dirname "$0")/setup/packages-osx.sh"
...
echo "Installing speech-to-text dependencies for OpenCode..."
if [[ "$DRY_RUN" == "true" ]]; then
	echo "[DRY RUN] Would install Python STT packages"
else
	uv pip install sounddevice soundfile numpy 2>/dev/null || pip install sounddevice soundfile numpy
	uv pip install useful-moonshine-onnx@git+https://github.com/moonshine-ai/moonshine.git#subdirectory=moonshine-onnx 2>/dev/null || echo "Moonshine install failed, will retry later"
fi
