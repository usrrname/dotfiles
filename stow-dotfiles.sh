...
setup_local_files() {
	if [[ "$OS" == "Darwin" ]]; then
		if [[ ! -f ~/.aliasrc-osx ]]; then
			local example="$SCRIPT_DIR/macos/zsh/.aliasrc-osx.example"
			if [[ -f "$example" ]]; then
				cp "$example" ~/.aliasrc-osx
				echo "   Created ~/.aliasrc-osx from template"
			fi
		else
			echo "   ~/.aliasrc-osx already exists, skipping"
		fi
	fi
}
